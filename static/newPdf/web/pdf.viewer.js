  ...
  load: function load(pdfDocument) {
    ...
    pdfDocument.getPageLabels().then(function (labels) {
      ...
      if (numLabels !== _this5.pagesCount) {
        console.error('The number of Page Labels does not match ' + 'the number of biz in the document.');
        return;
      }
    ...
    pdfDocument.getMetadata().then(function (_ref5) {
      ...
      /**
       * 如果是search 主动出发 highlight
       * @type {boolean}
       */
      let isSearch = checkIsSearch();
      if (isSearch) {
        const e = document.createEvent("MouseEvents");
        e.initEvent("click", true, true);
        // document.getElementById('findMatchCase').setAttribute('checked', 'checked');
        document.getElementById("findHighlightAll").dispatchEvent(e)
      }
      if (pdfTitle) {
...
function () {
  ...
  _createClass(PDFFindController, [{
    ...
    value: function _calculateWordMatch(query, pageIndex, pageContent, entireWord, isKeyword) {
      var matchesWithLength = [];
      var queryArray = query.match(/\S+/g) || [];
      if (isKeyword) {
        queryArray.push(isKeyword)
      }
    ...
    value: function _calculateMatch(pageIndex) {
      ...
      if (!isSearch) {
        ...
        if (!getMatchArray().length && query.length === 0) return ;
      }
      if (!caseSensitive) {
      ...
      /**
       * _calculatePhraseMatch 段落匹配，包括空格的匹配
       * _calculateWordMatch   单词匹配，会按空格切割匹配数组
       */
      // console.log('phraseSearch', phraseSearch, pageIndex)
      if (isSearch) {
        this._calculateWordMatch(getMatchArray().join(' '), pageIndex, pageContent, entireWord, query);
      } else {
        if (phraseSearch) {
      ...
      // console.log(this._resumePageIdx, '_resumePageIdx')
    ...
    value: function _updateAllPages() {
       ...
       * @type {boolean}
       */
      // const isSearch = checkIsSearch();
      // const inputValue = document.getElementById("findInput").value;
      // if (isSearch && !inputValue) {
        // this._resumePageIdx = 0;
        // const e = document.createEvent("MouseEvents");
        // e.initEvent("click", true, true);
        // document.getElementById("findHighlightAll").dispatchEvent(e)
      // }
...
function () {
  ...
  _createClass(BaseViewer, [{
    ...
    set: function set(rotation) {
      if (!(0, _ui_utils.isValidRotation)(rotation)) {
        throw new Error('Invalid biz rotation angle.');
      }
...
function () {
  ...
  _createClass(TextLayerBuilder, [{
    ...
    value: function _renderMatches(matches) {
      ...
      // console.log(selectedMatchIdx, 'selectedMatchIdx')
      ...
      function appendTextToDiv(divIdx, fromOffset, toOffset, className) {
        ...
        let keywordName = '';
        if (checkIsSearch()) {
          keywordName = getMatchArray().includes(content) ? ' keyword' : highlightAll ? '' : ' notKeyword';
        }
        if (className) {
          let span = document.createElement('span');
          span.className = className + keywordName;
...
PDFPrintService.prototype = {
  layout: function layout() {
    ...
    if (!hasEqualPageSizes) {
      console.warn('Not all biz have the same size. The printed ' + 'result may be incorrect!');
    }
...
function checkIsSearch () {
  return Boolean(String(GetQueryValue('search')) === 'search')
};
...
function getMatchArray () {
  const str = localStorage.pdf_wab_search_data || '';
  let arr = str.split('||');
  let words = [];
  const findMatchCase = document.getElementById("findMatchCase").checked;
  arr.forEach(item => {
    if (item) {
      let word = item.split('|v|')[0];
      words.push(findMatchCase ? word : word.toLowerCase())
    }
  })
  return words;
}
