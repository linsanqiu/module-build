    ...
    export default {
                                ...
                                (obj, i) => {
                                    ...
                                    switch (type) {
                                        case 1:
                                            ...
                                            routerPath = `/audit/news/tag/operate-handle`;
                                            break;
                                        case 2:
                                            operateUrl = `/tm/getNewsInfoById`;
                                            routerPath = `/audit/news/tag/operate-audit`;
                                            break;
                                        case 3:
                                            operateUrl = `/tm/getNewsInfoById`;
                                            routerPath = `/audit/news/tag/operate-verify`;
                                            break;
                                        case 5:
                                            operateUrl = `/tm/getNewsInfoById`;
                                            routerPath = `/audit/news/tag/operate-view`;
                                            break;
                                        default:
                                            operateUrl = `#type=删除`;
                                        ...
                                        on: {
                                            click: (e) => {
                                                ...
                                                if (type === 4) {
                                                    ...
                                                    window.sessionStorage.setItem(`tag_flipObj_${new_type}`, JSON.stringify(flipObj));
                                                    let router_query = {
                                                        type,
                                                        desc,
                                                        uid,
                                                    ...
                                                    window.sessionStorage.setItem(`tag_route_query_${new_type}`, JSON.stringify(router_query));
                                                    if (type === 5) {
        ...
        methods: {
            ...
            getCommonTableDatas (options = {}, debug = false) {
                ...
                // } = options;
                // 标签数量异常资讯列表
                options.newsListType = "2";
                this.tableLoading = true;
                let url = `/tm/getNoTagNewsLogVoList`;
                this.$http
                .post(url, options)
                .then(res => res.data)
                .then((json)=>{
            ...
            globalBindKeyboardShortcuts() {
                ...
                if (bindFlag === "true") {
                    console.log(`tag, body no need binding again!`);
                } else {
                    console.log(`tag, body only need binding once!`);
        ...
        mounted() {
            ...
            if (isGobalBindKS) {
                // only bind once
                console.warn(`tag, KeyboardShortcuts, only need bind once!`, isGobalBindKS);
            } else {
                // init bind
                console.info(`tag, KeyboardShortcuts, bind once!`, isGobalBindKS);
