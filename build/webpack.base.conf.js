...
// 汇总子模块的alias
const files = fse.readdirSync(resolve('src/biz/'));
files.forEach(item => {
    // 判定是否存在
    const exits = fse.pathExistsSync(resolve('src/biz/' + item + '/config.js'));
    if (exits) {
        const a = require('../src/biz/' + item + '/config.js');
        if (a.alias) {
