...
// 复制文件
const copyPlugin = [];
const staticExits = fse.pathExistsSync(
...
const exits = fse.pathExistsSync(resolve('src/biz/' + childName + '/config.js'));
if (exits) {
  const a = require('../src/biz/' + childName + '/config.js');
  if (a.alias) {
    for (const i in a.alias) {
      aliasMap[i] = resolve(a.alias[i]);
    }
  }
}
const sysConfigExits = fse.pathExistsSync(
