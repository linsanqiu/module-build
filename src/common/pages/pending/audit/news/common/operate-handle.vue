<template>
    <section data-body="common-handle">
                    ...
                    <h-row class-name="common-handle-padding">
                        <h-col span="4">
                            <h-button
                                ...
                                ref="common_handle_save"
                                data-btn="common_handle_save"
                        ...
                        <h-col span="4" v-if="commonHandle.openFlag">
                            <h-button
                                :disabled="isEmptyNewsTitle"
                                ref="common_handle_open"
                                data-btn="common_handle_open"
                        ...
                        <h-col span="4" v-else>
                            <h-button
                                :disabled="isEmptyNewsTitle"
                                ref="common_handle_close"
                                data-btn="common_handle_close"
                                @click="clickShowResureModal(`close`)">
                                <h-icon name="lock" />屏蔽
                            </h-button>
                        </h-col>
                        <h-col span="6" v-if="isNewsHandleFinsihed">
                            <h-button
                                :disabled="isEmptyNewsTitle"
                                ref="common_handle_passed"
                                data-btn="common_handle_passed"
                        ...
                        <h-col span="4">
                            <h-button
                                ...
                                ref="common_handle_prev"
                                data-btn="common_handle_prev"
                        ...
                        <h-col span="4">
                            <h-button
                                :disabled="isLastNews"
                                ref="common_handle_next"
                                data-btn="common_handle_next"
                    ...
                    <h-row class-name="common-handle-padding">
                        <h-col span="24" v-if="!isEmptyCommonLogs">
                            ...
                            <div
                                ...
                                style="width: 80%;">
                                <div id="audit-common-handle-logs"></div>
                            </div>
                        </h-col>
                    </h-row>
                </h-col>
            ...
            <h-row class-name="common-handle-padding">
                <h-col span="24">
                    <div
                        ...
                        style="width: 100%;">
                        <div id="audit-common-handle-text"></div>
                    </div>
                </h-col>
            </h-row>
        </div>
        ...
        <h-msgBox
            ...
            <h-row>
                <h-col span="24">
                    ...
                    <br/>
                    <!-- <span>行业类别 ID: <strong>{{labelCategoryId}}</strong></span>
                    <br/> -->
       ...
       <!-- <section data-uid="menus" class="hidden-menus">
            <h-row>
                <h-col span="24">
                    <button data-uid="menus-add" class="hidden-menus-btn">
                        添加标签
                    </button>
                </h-col>
                <h-col span="24">
                    <button data-uid="menus-hot" class="hidden-menus-btn">
                        添加热词
                    </button>
                </h-col>
            </h-row>
    ...
    export default {
        data () {
            return {
                ...
                // bind once
                isBindKeyboardShortcutKeys: true,
                // modal button ??? OK
                isShieldValid: true,
        ...
        methods: {
            ...
            sessionRouterQuery(id = ``) {
                let router_query = JSON.parse(window.sessionStorage.getItem(`common_route_query_handle`));
                let {
                    type,
                    desc,
                    // uid,
                ...
                window.sessionStorage.setItem(`common_route_query_handle`, JSON.stringify(new_router_query));
            },
            sessionFilterObj(id = ``) {
                let pageFlipObj = JSON.parse(window.sessionStorage.getItem("common_flipObj_handle"));
                // this.pageFlipObj = pageFlipObj || {};
                this.pageFlipObj.newsId = id;
                // pageFlipObj
                let new_pageFlipObj = Object.assign({}, this.pageFlipObj);
                window.sessionStorage.setItem(`common_flipObj_handle`, JSON.stringify(new_pageFlipObj));
            },
            reSureCancel() {
            ...
            onShieldSelectChange(type = `shieldTagReason`) {
                let value = this[type];
                // let value = this.commonHandle[type];
                // console.log(`shield value =`, value);
                if (value !== "") {
            ...
            keyboardShortcutKeys() {
                ...
                if (bindFlag === "true") {
                    console.log(`only need binding not once!`);
                } else {
                    console.log(`only need binding once!`);
                    body.dataset.bindFlag = "true";
                    // bind once
                    // body.addEventListener("keyup", keyupNamedCallback, true);
                    body.addEventListener("keyup", keyupNamedCallback, false);
                    // useCapture defaults to false.
                    body.addEventListener("keydown", keydownNamedCallback, false);
                }
            },
            keyupNamedCallback(e) {
            ...
            keydownNamedCallback(e) {
                ...
                if(e.which === 83 && isCtrl) {
                    if (that.onClickButton) {
                        ...
                        console.log(`common handle & save`);
                        // that.$refs.common_handle_save.$emit("click");
                        // console.log(`common handle & save btn click ok!`);
                    }
                }
                if(e.which === 68 && isCtrl) {
                    if (that.clickGetNewsInfos) {
                        if (!that.isFirstNews) {
                            ...
                            // that.$refs["common_handle_prev"].click();
                            let prev_btn = that.$refs.common_handle_prev;
                            console.log(`common handle & prev_btn`, prev_btn);
                            if (prev_btn) {
                                // prev_btn.click();
                                // prev_btn.$emit("click");
                                prev_btn.$el.click;
                                console.log(`common handle & prev btn click ok!`);
                            } else {
                                console.error(`common handle & prev btn click error!`);
                            }
                            // that.$refs.common_handle_prev.click();
                            // that.$refs.common_handle_prev.$emit("click");
                        } else {
                            // btn bug
                ...
                if(e.which === 70 && isCtrl) {
                    if (that.clickGetNewsInfos) {
                        if (!that.isLastNews) {
                            ...
                            // that.$refs["common_handle_next"].click();
                            let next_btn = that.$refs.common_handle_next;
                            console.log(`common handle & next_btn`, next_btn);
                            if (next_btn) {
                                // next_btn.click();
                                // next_btn.$emit("click");
                                next_btn.$el.click;
                                console.log(`common handle & next btn click ok!`);
                            } else {
                                console.error(`common handle & next btn click error!`);
                            }
                            // that.$refs.common_handle_next.click();
                            // that.$refs.common_handle_next.$emit("click");
                        } else {
                            // btn bug
                ...
                if(e.which === 88 && isCtrl) {
                    ...
                    //     console.log(`common handle & passed`);
                    //     // that.$refs.common_handle_passed.$emit("click");
                    //     // console.log(`common handle & passed btn click ok!`);
                    // }
                }
            },
                    ...
                    if(status != this.$api.SUCCESS){
                        ...
                        if (body === null) {
                            if (type === "prev") {
                                ...
                                console.log(`this.isFirstNews = `, this.isFirstNews);
            ...
            reSureDelete () {
                ...
                //         // this.$hMessage.error(`删除标签失败!`);
                //     }, 500);
                // }, 1000);
            },
            returnParentPage () {
                // re-direct
                let path = `/audit/news/common/operate-handle`;
                this.$store.commit("DEL_TAB", path);
                this.$router.push(`/audit/news/common`);
            },
            ...
            shitClearableSelect(type = ``) {
                // ref="labelCategoryModal"
                // http://192.168.58.189:8080/hui/#/components/selectTree
                // console.log(`xxxxx`);
                // shiy api
                this[type] = `101`;
                this.$refs[type].focus();
            ...
            onChangeInput (type = ``) {
                ...
                // console.log(`value.trim() =`,  value.trim(),  value.trim().length);
            ...
            onClickButton (type = ``) {
                ...
                switch (type) {
                    ...
                    case "open":
                        ...
                        path = `/audit/news/common/operate-handle`;
                        break;
                    case "close":
                ...
                if (!xwbt) {
                    // console.log(`资讯标题 =`, xwbt, xwbt.length);
                    this.$hMessage.info(`资讯标题不可以为空!`);
                } else if (tags.length === 0 && type === "passed") {
            ...
            getLabelNames() {
                let that = this;
                let url = `/tm/getTagCategory?categoryId=101101101`;
                const fetchJSON = (url = `/tm/getTagCategory?categoryId=101`) => {
                    return fetch(url)
                    .then(res => res.json())
                    .then(
                        (json) => {
                ...
                async function getDatas(url = ``) {
                    try {
                        const datas = await fetchJSON(url);
                        // if (condition) {
                        //     that.$hMessage.info(`为查找到该标签的标签分类信息!`);
                        // } else {
                        //     that.$hMessage.info(`为查找到该标签的标签分类信息!`);
                ...
                if (labelCategoryId === "101") {
                    ...
                    .then((json)=>{
                        ...
                        if (status === 1) {
                            ...
                            that.$hMessage.info(`未查找到该标签的标签分类信息!`);
            ...
            asyncAddTag () {
                ...
                if (add_tag.includes("{")) {
                    ...
                    // console.log(`bqdm & tag_uid =`, tag_uid);
                    ...
                    if (uniqueTagsIds.includes(tag_uid)) {
                        ...
                        //     tag,
                        // });
                        // this.commonHandleTableDatas.push(tagObj);
                        setTimeout(() => {
            ...
            onSearchValueQueryChange(value = ``) {
                ...
                // console.log(`this.isEmptySearch 5=`, this.isEmptySearch);
            },
            ...
            getHandleInfos(uniqueNewsId = ``) {
                ...
                .then((json)=>{
                    ...
                    if(status != this.$api.SUCCESS){
                        ...
                        this.commonHandleTableDatas = listDatas;
                        // this.commonAllTags = commonAllTags;
                        this.uniqueTagsIds = uniqueTagsIds;
                        // console.log(`commonAllTags =`, JSON.stringify(commonAllTags, null, 4));
                        // console.log(`uniqueTagsIds=`, JSON.stringify(uniqueTagsIds, null, 4));
                    }
                    this.clickGetNewsId();
                    this.clickGetNewsLink();
                    // auto bind
                    // console.log(`init =`, this.$refs.common_handle_prev);
                    // console.log(`init =`, this.$refs.common_handle_next);
                    // let isBindKeyboardShortcutKeys = this.isBindKeyboardShortcutKeys;
                    // if (isBindKeyboardShortcutKeys) {
                    //     this.keyboardShortcutKeys();
                    //     this.isBindKeyboardShortcutKeys = false;
                    //     console.info(`bind once `, isBindKeyboardShortcutKeys)
                    // } else {
                    //     console.error(`bind once bug`, isBindKeyboardShortcutKeys)
                    // }
                    // this.tableLoading = false;
            ...
            init() {
                ...
                this.getHandleInfos();
                // 高亮
                // this.$store.commit("SET_ACTIVE_MENU_PATH", "/audit/news/common");
                // let that = this;
                // setTimeout(() => {
                //     that.clickGetNewsId();
                // }, 100);
        ...
        // no cache, no tab  activated
        activated() {
            // keep-alive 组件激活时调用。
            // console.log(`keep-alive 组件激活时调用`);
            // console.log(`common_handle tab 激活!`);
        },
    }
</script>
