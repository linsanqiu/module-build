	...
	<div id="detailBox"  :class="pdfInfo.processTypeCode == 4 || pdfInfo.processTypeCode == 5 ? '':'noBtn'">
		<windowDrag leftW="50%">
			<div class="box-left-content" slot="left">
				<ul id="pdfBox">
					<li class="input-box">
					 	<span>公告链接{{ pdfInfo.processTypeCode }}：</span>
					 	<span>						 		 
                			<a :title="pdfInfo.linkAddress" v-if="pdfInfo.linkAddress" :href="pdfInfo.linkAddress" target="_blank">{{pdfInfo.linkAddress}}</a>				               
					 		<i v-else>-</i>
					 	</span>
					...
					<li class="input-box">
					 	<span>公告内容：</span>
					 	<template v-if="pdfInfo.extension == 'pdf'">
					...
					<template  v-if="pdfInfo.extension == 'pdf'">
						<iframe ref="pdfIframeInfo" name="pdfIframe" :src="pdfUrl" width="100%" :height="maxTableHeight-50" frameborder="0"></iframe>	
					</template>									
				</ul>				
			</div>
			<div class="box-right-content" slot="right" >
				<div class="productList" v-for="(itemArr,index) in tableTemplateData">
					<TableTemplate :viewPage="true" :isAdd="false" :templateData="tableTemplate" v-model="tableTemplateData[index]" :onTitle="onTableName">
					</TableTemplate>
				</div>
				<div class="saveBox">
...
<script>
	...
	export default{
		...
		methods:{
			...
			getDetailData(){
				 this.$http.get('/pic/audit/anncData/getAnncDetail?taskId='+this.taskId).then((res) => {
		        	let data = res.data;
