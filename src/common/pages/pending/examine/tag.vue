		...
		<search-form>
			<ul slot="content">
							...
							<h-select placeholder="处理人" v-model="intHandleUser" filterable remote  :remote-method="getHandleUserDict" @on-change="getHrefUrl" 
								...
								:loading="isReqHandleUser">
									<template v-for="(data,key) in aryHandleUserList">					
										<h-option :value="data.id" :key="data.id">
				...
				<li class="search-wrapper-but">
					...
					<h-button type="primary" @click="onTaskDistri" v-if="activeRoutersButton.indexOf('TagBathchDistri') != -1">批量分配</h-button>
					<a class="h-btn h-btn-primary" :href="exportUrl" target="_blank" v-if="activeRoutersButton.indexOf('TagExport') != -1">导出</a>
					<h-button type="primary" @click="onOneKeyTaskDistri" v-if="activeRoutersButton.indexOf('TagOneKeyDistri') != -1" >自动分配</h-button>
				</li>
			</ul>
		</search-form>				
		<!--数据列表-->
			...
			<template v-if="tagConfirmForm.confirmStep==0" >
				<div class="confirm-form-content">
					...
					<div class="search" >
						...
						<h-select v-model="strConfirmTagType" :clearable="false" @on-change="onSearchSynonTagList" placeholder="请选择" disabled>		
							<template v-for="(data,key) in aryTagTypeList">			
								<h-option :value="data.category_id" :key="data.category_id">{{data.category_name}}</h-option>	
							</template>
						</h-select>
				...
				<div slot="footer" class="confirm-form-footer" >
					...
					<h-button type="primary" @click="onSureNextOne" :loading="isNext" >下一步</h-button>
				</div>				
			</template>
			<template v-else-if="tagConfirmForm.confirmStep==1">
				<div class="confirm-form-content">
				...
				<div slot="footer" class="confirm-form-footer" >
					...
					<h-button type="primary" @click="onSureNextTwo" v-if="!isReqRegister">下一步</h-button>
					<h-button type="primary" v-else>注册中</h-button>
					
				</div>	
			</template>
			<template v-else>
				<h-form :label-width="90" class="confirm-form">	
					<h-form-item label="通知内容：">
						<span>{{tagConfirmForm.noticeMessage}}</span>
					</h-form-item>			
				</h-form>
				<div slot="footer" class="confirm-form-footer" >
					<h-button @click="onSureUpOne">上一步</h-button>
					<h-button type="primary" @click="onSubmitConfirmTag" :loading="isReqConfirmTag" :disabled="isSubmit">提交</h-button>
				</div>	
			</template>
		</h-msgBox>
		<!--拒绝添加窗口-->
			...
			<template v-if="tagRefuseForm.refuseStep == 1">
				<div class="confirm-form-content" v-if="tagRefuseForm.refuseReason != 3">
					<template v-if="tagRefuseForm.refuseReason == 2">
						...
						<div class="search" >
							...
							<span class="title title-margin">标签类别：</span>
							<h-select v-model="strSynonTagType" :clearable="false" @on-change="onSearchSynonTagList" placeholder="请选择" disabled>		
								<template v-for="(data,key) in aryTagTypeList">			
									<h-option :value="data.category_id" :key="data.category_id">{{data.category_name}}</h-option>	
								</template>
							</h-select>				
					...
					<template v-if="tagRefuseForm.refuseReason == 1">
						...
						<div class="search" >
							...
							<span class="title title-margin">标签类别：</span>
							<h-select v-model="strExistTagType" :clearable="false" @on-change="onSearchSynonTagList" placeholder="请选择" disabled>		
								<template v-for="(data,key) in aryTagTypeList">			
									<h-option :value="data.category_id" :key="data.category_id">{{data.category_name}}</h-option>	
								</template>
							</h-select>			
				...
				<template v-else>
					<h-form :model="tagRefuseForm" :label-width="95" :rules="descriptionRule" ref="tagRefuseForm" class="confirm-form">							
						<h-form-item label="通知内容：" prop="noticeMessage">
							<div>{{tagRefuseForm.noticeMessage}}</div>
						</h-form-item>
				...
				<div slot="footer" class="confirm-form-footer"  
				v-if="tagRefuseForm.refuseReason != 3">
					<h-button @click="onUpStepRefuse">上一步</h-button>
					<h-button type="primary" @click="onNextStepRefuse" 
					:loading="isNextRefuse">下一步</h-button>
				</div>								
			</template>
			<template v-if="tagRefuseForm.refuseStep == 2">					
				<h-form :label-width="90" ref="noticeForm">
					<h-form-item label="同义词：" prop="synonym" 
					v-if="tagRefuseForm.refuseReason==2">
						<h-input v-model.trim="tagRefuseForm.synonym" placeholder="请输入..." autocomplete="off">
						</h-input>
					</h-form-item>
					<h-form-item label="通知内容：">
						<span>{{tagRefuseForm.noticeMessage}}</span>
					</h-form-item>			
				</h-form>
				<div slot="footer" class="confirm-form-footer" >
					<h-button @click="onUpStepRefuseOne">上一步</h-button>
					<h-button type="primary" @click="onSubmitConfirmTag" :loading="isReqConfirmTag" :disabled="isSubmit">提交</h-button>
				</div>		
			</template>		
		</h-msgBox>	
		<h-msgBox title="申请用户" v-model="isShowApplyUserBox"  @on-close="onCloseApplyUserBox" class-name="vertical-center-modal" :top="0" width='250'>
			<div >				
				<p class="user-content">用户名：{{applyUserObj.userName}}</p>
				<p class="user-content">所属公司：{{applyUserObj.corpName}}</p>
				<p class="user-content">行业：{{applyUserObj.corpIndustry}}</p>
			</div>
			<div slot="footer" >					
			</div>	
		</h-msgBox>
		<h-msgBox title="标签搜索" v-model="isShowViewBox"  @on-close="onCloseViewBox" class-name="vertical-center-modal" :top="0" width="700">
			<div class="confirm-form-content">
				<div class="search" >					
					<span class="title">标签名称：</span><h-input placeholder="请输入..." v-model.trim="strExistTagKeyword"  @on-keyup.enter="onSearchSynonTagList" icon="search" @on-click="onSearchSynonTagList"></h-input>
					<span class="title title-margin">标签类别：</span>
	...
	export default {
				...
				aryHeaderData: [				
					{
						...
						render: (h, params) => {
							...
							let confirmBtn = null;
							let refuseBtn = null;
							if(this.activeRoutersButton.indexOf('TagDistri') != -1&&params.row.handleStatus != 0){
							//待分配和待处理展示待分配
								distriBtn = h('i',{
									class: {
										'iconfont': true,
							...
							if(this.activeRoutersButton.indexOf('TagAdd') != -1&&params.row.handleStatus == 1){
							//待处理展示确认添加和拒绝添加
								confirmBtn = h('i',{
									class: {
										'iconfont': true,
							...
							if(this.activeRoutersButton.indexOf('TagRefuse') != -1&&params.row.handleStatus == 1){
								refuseBtn = h('i',{
									class: {
										'iconfont': true,
		...
		methods: {	
			...
			getSearchParams(){
				...
				let urlParams = utils.searchParams(tagType,handleStatus,handleResult,this.strTagKeyword,handleUser,1,this.applyTimeBegin,this.applyTimeEnd,this.handlerTimeBegin,this.handlerTimeEnd,this.strTagId);			
				return urlParams;
			},
			...
			onTagConfirmBtn(row){
				utils.checkhandler(row,(data)=>{						
					...
					this.tagConfirmForm.tagName=row.tagName;
					this.addType=1;
			...
			onSureNextOne(){				
				...
				utils.checkTag(businessId,tagValue,categoryId,(res)=>{
					...
					this.tagConfirmForm.isexists =res.data.isexists; 
					if(res.data.isexists){
						this.tagConfirmForm.result=res.data.result;						
						let msgData = this.getNoticeMsgData();					
						utils.noticeMsg(msgData,(data)=>{
							this.isNext=false;
							if(!this.httpResValid(data)) return;
							this.tagConfirmForm.noticeMessage=data.data ? data.data : '';
							this.tagConfirmForm.confirmStep=2;
							this.isSubmit=false;
						})
			...
			onSureNextTwo(){	
				...
				utils.register(dataJson,(res)=>{
					...
					if(!this.httpResValid(res)) return;
					this.tagConfirmForm.result = res.data;
					let msgData = this.getNoticeMsgData();					
					utils.noticeMsg(msgData,(data)=>{						
						if(!this.httpResValid(data)) return;
						this.tagConfirmForm.noticeMessage=data.data ? data.data : '';
			...
			onSureUpOne(){
				if(this.tagConfirmForm.isexists){
					this.tagConfirmForm.confirmStep=0;
					return;
				}
				this.tagConfirmForm.confirmStep=1;
			...
			onTagRefuseBtn(row){
				utils.checkhandler(row,(data)=>{						
					...
					this.tagRefuseForm.refuseReason='';					
					...
					this.tagRefuseForm.tagName= row.tagName;				
			...
			getNoticeMsgData(){
				...
				if(this.addType==1){
					data = {
						handleResult: this.tagConfirmForm.handleResult,
						refuseReason: this.tagConfirmForm.refuseReason,
						synTagName: tagName,				   
						tagId: this.tagConfirmForm.tagId,
						//tagId:25938383508302170,
						tagName:this.tagConfirmForm.tagName
					};	
				}else{
					data = {
						handleResult: this.tagRefuseForm.handleResult,
						refuseReason: this.tagRefuseForm.refuseReason,
						synTagName: tagName,				   
						tagId: this.tagRefuseForm.tagId,
						//tagId:25938383508302170,
						tagName:this.tagRefuseForm.tagName
					};	
				}
				return data;
			},
			...
			onRefuseReasonBox(){
				...
				if(value==1 || value==2){//标签已存在或者有同义标签
					...
					utils.noticeMsg(msgData,(data)=>{						
						if(!this.httpResValid(data)) return;
						this.tagRefuseForm.noticeMessage=data.data ? data.data : '';
						this.isSubmit=false;
					})									
				}			
			...
			onNextStepRefuse(){
				...
				if(this.isNextRefuse) return;
				this.isNextRefuse = true;				
				let tagValue = this.synonTagObj.tag_value;
				let businessId = this.synonTagObj.tag_id;
				let categoryId = this.synonTagObj.categorys[0].categoty_id;				
				utils.checkTag(businessId,tagValue,categoryId,(res)=>{
					...
					if(!this.httpResValid(res)) return;
					if(res.data.isexists){
						this.tagRefuseForm.result=res.data.result;
						let msgData = this.getNoticeMsgData();					
						utils.noticeMsg(msgData,(data)=>{
							this.isNextRefuse = false;						
							if(!this.httpResValid(data)) return;
							this.tagRefuseForm.noticeMessage=data.data ? data.data : '';
							this.tagRefuseForm.refuseStep=2;
							this.isSubmit=false;
						})
			...
			onSubmitConfirmTag(){
				...
				}else if(this.tagRefuseForm.refuseReason==2){
			...
			submit(){
				...
				if(this.addType==1){
					dataConfirm = { 
						handleResult: this.tagConfirmForm.handleResult,
						tagId:this.tagConfirmForm.tagId,
						noticeMessage:this.tagConfirmForm.noticeMessage,
						refuseReason:'',
						remark:this.tagConfirmForm.remark,
						relationTagId:this.synonTagObj.tag_id,
						relationTagName:this.synonTagObj.tag_value,
						result:this.tagConfirmForm.result
					 };
				}else{
					dataConfirm = { 
						handleResult: this.tagRefuseForm.handleResult,
						tagId:this.tagRefuseForm.tagId,
						noticeMessage:this.tagRefuseForm.noticeMessage,
						refuseReason:this.tagRefuseForm.refuseReason,
						remark:this.tagRefuseForm.remark,
						relationTagId:this.synonTagObj.tag_id,
						relationTagName:this.synonTagObj.tag_value,
						result:this.tagRefuseForm.result
					 };
				}
			...
			//获取申请用户的详细信息，弹窗展示
			getApplyUser(row){			
				utils.applyUser(row,(data)=>{						
					if(!this.httpResValid(data)) return;
					let tmpObj = data;						
					this.applyUserObj = tmpObj.data ? tmpObj.data : {}; 
					this.isShowApplyUserBox=true;		
				})	
			},
			onCloseApplyUserBox(){
				this.isShowApplyUserBox=false;
				this.applyUserObj={};
			},			
			/*初始化，加载页面初始的数据*/
			init(){							
