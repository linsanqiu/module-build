...
<script>
     ...
     * @modified 2018.01.06
     *
     * @description  BID 异常资讯列表 BidTagNews
     * @augments
     * @example
     *
     */
    ...
    export default {
                                ...
                                (obj, i) => {
                                    ...
                                    switch (type) {
                                        case 1:
                                            ...
                                            routerPath = `/audit/news/bid-tag/operate-handle`;
                                            break;
                                        case 2:
                                            operateUrl = `/tm/getNewsInfoById`;
                                            routerPath = `/audit/news/bid-tag/operate-audit`;
                                            break;
                                        case 3:
                                            operateUrl = `/tm/getNewsInfoById`;
                                            routerPath = `/audit/news/bid-tag/operate-verify`;
                                            break;
                                        case 5:
                                            operateUrl = `/tm/getNewsInfoById`;
                                            routerPath = `/audit/news/bid-tag/operate-view`;
                                            break;
                                        default:
                                            operateUrl = `#type=删除`;
                                        ...
                                        on: {
                                            click: (e) => {
                                                ...
                                                if (type === 4) {
                                                    ...
                                                    window.sessionStorage.setItem(`bid-tag_flipObj_${new_type}`, JSON.stringify(flipObj));
                                                    let router_query = {
                                                        type,
                                                        desc,
                                                        uid,
                                                    ...
                                                    window.sessionStorage.setItem(`bid-tag_route_query_${new_type}`, JSON.stringify(router_query));
                                                    if (type === 5) {
                                                         this.$router.push(options);
                                                    } else {
                                                        // tryLock & POST
        ...
        methods: {
            ...
            getCommonTableDatas (options = {}, debug = false) {
                ...
                // } = options;
                // 重名资讯列表
                options.newsListType = "3";
                this.tableLoading = true;
                let url = `/tm/getErrorHumanTagNewsLogVoList`;
                this.$http
                .post(url, options)
                .then(res => res.data)
                .then((json)=>{
                    ...
                    if(status != this.$api.SUCCESS){
                        console.log(`BID 异常资讯列表 json =`, JSON.stringify(json, null, 4));
                        // this.$hMessage.error(msg);
                        this.initTableDatas = [];
                ...
                }).catch(err => console.error(`BID 异常资讯列表 error`, err));
            },
            getWindowMaxWidth() {
            ...
            globalBindKeyboardShortcuts() {
                ...
                if (bindFlag === "true") {
                    console.log(`bid-tag, body no need binding again!`);
                } else {
                    console.log(`bid-tag, body only need binding once!`);
        ...
        mounted() {
            ...
            if (isGobalBindKS) {
                // only bind once
                console.warn(`bid-tag, KeyboardShortcuts, only need bind once!`, isGobalBindKS);
            } else {
                // init bind
                console.info(`bid-tag, KeyboardShortcuts, bind once!`, isGobalBindKS);
