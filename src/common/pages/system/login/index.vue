...
export default {
      ...
      this.$http
        ...
        .then((res) => {
          ...
          if (obj.status == this.$api.SUCCESS) {
            ...
            let userInfo = encodeURIComponent(JSON.stringify(data.user));
            let userMenu = encodeURIComponent(JSON.stringify(data.menu));
            let key = this.$md5(userInfo + "key" + userMenu); //校验cookie md5秘钥
  ...
  mounted() {
    ...
    this.isRememberUserName =
      localStorage.isRememberUserName === "true" ? true : false;
    if (this.isRememberUserName) {
