<template>
  ...
  >
    <template v-if="pageType=='HANDLE' || pageType=='ADD'">
      ...
      >
        <h-option
          ...
          :key="item.dictEntry"
        >{{item.entryName ? item.entryName : '-'}}</h-option>
      </h-select>
      <h-select
        autoPlacement
        multiple
      ...
      ></h-date-picker>
      <h-checkbox-group v-if="formItemType == 'checkbox'" v-model="commonFilters[itemName]">
        <h-checkbox
    ...
    <template
      v-if="pageType=='VIEW' || pageType=='SPECIALVIEW' || pageType=='SPECIALEDIT'|| pageType=='SPECIALADD'"
    >
      <div
        class="viewPage"
        v-if="!linkType"
        v-html="commonFilters[itemName]"
      >{{commonFilters[itemName] ? commonFilters[itemName] : '-'}}</div>
      <div class="viewPage linkUrlItem" v-else>
        <a
          :href="commonFilters[itemName]"
          target="_blank"
        >{{commonFilters[itemName] ? commonFilters[itemName] : ''}}</a>
      </div>
    </template>
  </h-form-item>
</template>
