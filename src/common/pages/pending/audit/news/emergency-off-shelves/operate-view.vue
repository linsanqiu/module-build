                    ...
                    <div v-if="!isNeedReturnOrigin">
                        <h-col span="4">
                            <h-button
                                ...
                                ref="emergency-off-shelves_view_prev"
                                data-btn="emergency-off-shelves_view_prev"
                        ...
                        <h-col span="4">
                            <h-button
                                :disabled="isLastNews"
                                ref="emergency-off-shelves_view_next"
                                data-btn="emergency-off-shelves_view_next"
                            ...
                            <h-row class-name="common-handle-padding" v-if="!isBidTagsEmpty">
                                <h-col span="24">
                                    <section>
                                        ...
                                        <h-row class-name="bid-table-content">
                                            <h-col span="24">
    ...
    export default {
                ...
                commonHandleTableCols: [
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            ...
                            // console.log(`labelWeight =`, labelWeight, typeof(labelWeight));
                            let isLowLabelWeight = false;
                            if (labelWeight < 0.34 && this.isWhiteListType) {
                                // this.ShowLowLabelWeightColor = true;
                                isLowLabelWeight = true;
                            }
                            return h("span",
                                {
                                    attrs: {
                                        ...
                                        title: isLowLabelWeight ? "低于0.34的标签过滤掉，不推送到推荐系统" : "",
                                    },
                                    "class": {
                                        // "showLowLabelWeightColor": true,
                                        "show-low-label-weight-color": isLowLabelWeight,
                                    },
                                    on: {
                                        // click: this.clickHandler,
                                    },
        ...
        methods: {
            ...
            sessionRouterQuery(id = ``) {
                let router_query = JSON.parse(window.sessionStorage.getItem(`emergency-off-shelves_route_query_view`));
                let {
                    type,
                    desc,
                    // uid,
                ...
                window.sessionStorage.setItem(`emergency-off-shelves_route_query_view`, JSON.stringify(new_router_query));
            },
            sessionFilterObj(id = ``) {
                let pageFlipObj = JSON.parse(window.sessionStorage.getItem("emergency-off-shelves_flipObj_view"));
                // this.pageFlipObj = pageFlipObj || {};
                this.pageFlipObj.newsId = id;
                // pageFlipObj
                let new_pageFlipObj = Object.assign({}, this.pageFlipObj);
                window.sessionStorage.setItem(`emergency-off-shelves_flipObj_view`, JSON.stringify(new_pageFlipObj));
            },
            // shortcut keys
            ...
            returnParentPage () {
                let path = `/audit/news/emergency-off-shelves/operate-view`;
                ...
                if (originPath) {
                    ...
                    this.$router.push(`/audit/news/emergency-off-shelves`);
                }
                //this.$router.push(`/audit/news/emergency-off-shelves`);
            },
            // filter methods
            ...
            getHandleInfos(uniqueNewsId = ``) {
                ...
                .then((json)=>{
                    ...
                    if(status != this.$api.SUCCESS){
                        ...
                        if (dataType === 0) {
                            this.isWhiteListType = true;
                        } else {
                            this.isWhiteListType = false;
                        }
            ...
            autoReturnOrigin() {
                ...
                if (originPath) {
                    store.commit('SAVE_TAB_NAME',{  path: this.$route.path, name: '资讯详情'});
                    store.commit('SAVE_ACTIVE_MENU', originPath),
                    this.isNeedReturnOrigin = true;
                } else {
                    store.commit('SAVE_TAB_NAME',{  path: this.$route.path, name: '全部资讯-查看'});
