				...
				<search-form class="top-search">
					<ul slot="content">
						...
						<li class="search-wrapper-but">
							...
							<h-button type="primary" @click="batchSave" v-if="activeRoutersButton.indexOf('submit') != -1">批量提交</h-button>
							<h-button type="primary" @click="batchDel" v-if="activeRoutersButton.indexOf('delete') != -1">批量删除</h-button>
...
export default {
							...
							view = h('span', {
								...
								on: {
									click: () => {
										...
										localStorage.incident_search_view_data = JSON.stringify(obj)
										this.$router.push('/corpus/incident/view?eventCode='+ this.req.eventCode +'&newsId=' + row.id);
									}
								},
							})
						}
							...
							modify = h('span', {
								...
								on: {
									click: () => {
										...
										localStorage.incident_search_modify_data = JSON.stringify(obj)
										this.$router.push('/corpus/incident/detail?eventCode='+ this.req.eventCode +'&newsId=' + row.id);
									}
								},
							})
						}
								...
								on: {
									click: () => {
										this.$hMsgBox.confirm({
											title:'温馨提示',
											content:'确定删除该资讯：'+ row.infoId +'？',
											onOk:()=>{
												this.setBatchDel(row.id);
											}
										})
									}
								},
	...
	methods: {
		...
		batchSave(){
			let list = this.selectNews;
			if(list.length == 0){
				this.$hMessage.error('请选择需要提交的资讯！');
				return;
			}
			if(this.isGetList)return;
			this.isGetList = true;
			let url = '/tm/corpus/eventtagprocess/batchCommitEventTagProcessVo';
		...
		setBatchDel(data = null){
			if(this.isGetList)return;
			this.isGetList = true;
			let url = '/tm/corpus/eventtagprocess/batchDeletEventTagProcessVo';
		...
		getNews(go = false){
			...
			this.isGetList = true;
			let url = '/tm/corpus/eventtagprocess/conditionalQuery';
