		...
		<search-form>
			<ul slot="content">
				...
				<li class="search-wrapper-but">
					<h-button  v-if="activeRoutersButton.indexOf('AmbiguousWordQuery') != -1" type="primary" data-ref="init-btn" @click="onClickButton(`search`)">
						查询
					</h-button>
					<h-button v-if="activeRoutersButton.indexOf('AmbiguousWordCreate') != -1" 
						type="primary"
						data-ref="add-btn"
						@click="onClickButton(`add`)">
						新建
					</h-button>
					<h-button  v-if="activeRoutersButton.indexOf('AmbiguousWordImport') != -1" 
						type="primary"
						data-ref="upload-btn"
						@click="onClickButton(`upload`)">
						导入
					</h-button>
					<h-button  v-if="activeRoutersButton.indexOf('AmbiguousWordExport') != -1" 
						type="primary"
							...
							(obj, i) => {
								...
								switch (type) {
									case 0:  // 删除
										operateUrl = ``;
										routerPath = ``;
										showBtn = this.activeRoutersButton.indexOf('AmbiguousWordDelete') != -1 ? true : false;
										break;
									case 1:  //编辑
										operateUrl = `/tm/tag/getWordList`;
										routerPath = `/audit/news/ambiguous-word-maintenance/operate-handle`;
										showBtn = this.activeRoutersButton.indexOf('AmbiguousWordModify') != -1 ? true : false;
										break;
									// case 2:
									//     operateUrl = `/tm/getNewsInfoById`;
									//     routerPath = `/audit/news/ambiguous-word-maintenance/operate-view`;
