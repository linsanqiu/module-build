                    ...
                    <h-row class-name="common-handle-padding">
                        ...
                        <h-col span="6">
                            <h-button
                                ...
                                <h-icon name="success" />审核通过
                            </h-button>
                        </h-col>
                        <h-col span="6">
                            <h-button
                                @click="clickShowResureModal(`passed`)">
                                <h-icon name="success" />审核不通过
                            </h-button>
                        </h-col>
                        <h-col span="4">
                            <h-button
                                :disabled="isFirstNews"
                                ref="review_verify_prev"
                                data-btn="review_verify_prev"
                        ...
                        <h-col span="4">
                            <h-button
                                :disabled="isLastNews"
                                ref="review_verify_next"
                                data-btn="review_verify_next"
                ...
                <h-col span="12">
                    <h-row class-name="common-handle-padding">
                        <h-col span="24">
                            ...
                            <h-row class-name="common-handle-padding">
                                <h-col span="12">
                                    <span class="audit-common-lable">审核意见</span>
                                     <h-input
                                        aria-placeholder="审核意见"
                                        placeholder="请输入审核意见"
                                        style="width: 60%"
                                        ref="newsNotes"
...
/**
     ...
     * @modified 2018.01.06
     *
     * @description 审核 operate-vertify
     * @augments
     * @example
     *
     */
    ...
    export default {
        ...
        methods: {
            ...
            sessionRouterQuery(id = ``) {
                let router_query = JSON.parse(window.sessionStorage.getItem(`review_route_query_verify`));
                let {
                    type,
                    desc,
                    // uid,
                ...
                window.sessionStorage.setItem(`review_route_query_verify`, JSON.stringify(new_router_query));
            },
            sessionFilterObj(id = ``) {
                let pageFlipObj = JSON.parse(window.sessionStorage.getItem("review_flipObj_verify"));
                // this.pageFlipObj = pageFlipObj || {};
                this.pageFlipObj.newsId = id;
                // pageFlipObj
                let new_pageFlipObj = Object.assign({}, this.pageFlipObj);
                window.sessionStorage.setItem(`review_flipObj_verify`, JSON.stringify(new_pageFlipObj));
            },
            // shortcut keys
            ...
            returnParentPage () {
                let path = `/audit/news/review/operate-verify`;
            ...
            onClickButton (type = ``) {
                ...
                // console.log(`apiStatusCode =`, apiStatusCode);
                let options = {
                    newsId,
                    remark,
                };
                ...
                switch (type) {
                    case "passed":
                        ...
                        // 9 === 敏感资讯强制公开
                        if (apiStatusCode === 9) {
                            url = `/tm/publicByNewsId`;
                        } else if (apiStatusCode === 10) {
                            url = `/tm/shieldByNewsId`;
                        } else if (apiStatusCode === 3) {
                            // ???
                            url = `/tm/publicByNewsId`;
                            // url = `/tm/shieldByNewsId`;
                        }
                ...
                if (apiStatusCode !== 0) {
                    ...
                    // 处理状态
                    this.$hMessage.error(`处理状态 dataStatus = ${value}, 不是合法的!`);
                }
            },
            commonAudit (url, options, type) {
                ...
                .then((json)=>{
                    ...
                    if(status != this.$api.SUCCESS){
                        ...
                        let message = (type === "passed") ? "审核通过成功!" : "审核不通过成功!"
