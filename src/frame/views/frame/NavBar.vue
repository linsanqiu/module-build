<template>
    <div class="tabs">
        <div id="tab-right-menu" class="rightmenu" v-show="isRightMenu" :style="rightMenuStyle"
             @click.stop="function () {}">
        ...
        <div class="menu_box">
            ...
            <div class="tabs-list" ref="tabsList">
                <ul ref="tabsListUl" :style="navStyle" @mousemove="mousemovefn">
                    <li :class="activeTabPath == '/home' ? 'active' : '' " @click="onHome"
                        @contextmenu.prevent="contextmenu(-1)">
                        <h-icon class="icon" name="homepage_fill"></h-icon>
                        首页
                    </li>
                    <draggable v-model="tabList" :options="dragOptions" @end="draggableUpdate"
                               @update="draggableUpdate">
                        <transition-group name="no" class="list-group" tag="div">
                            <li v-for="(data,key) in tabList"
                                :class="encrypt(tabList[key]) ? 'active general' : 'general' "
                                @contextmenu.prevent="contextmenu(key)" @click="changeTab(routersUrl[pathName(data)])"
                                :key="data" :keys="key">
...
export default {
    ...
    methods: {
        ...
        tabsOnresize() {
            const ulwidth = this.$refs.tabsListUl && this.$refs.tabsListUl.offsetWidth ? this.$refs.tabsListUl.offsetWidth : 0;
            const width = this.$refs.tabsList && this.$refs.tabsList.offsetWidth ? this.$refs.tabsList.offsetWidth : 0;
            if (width < ulwidth) {
        ...
        closeTabs(type) {
            ...
            const url = this.intRightMenuIndex != -1 ? this.routersUrl[pathName(tabList[this.intRightMenuIndex])] : '';
            const activeTabPath = this.$md5(this.activeTabPath);
            let isPush = false;
            if (type == 'right') {
        ...
        setOffset(name) {
            const ulwidth = this.$refs.tabsListUl.offsetWidth;
            const width = this.$refs.tabsList.offsetWidth;
        ...
        closeTab(key, isPush = true) {
            const url = this.tabList[key];
            const name = pathName(url);
            const msg = this.tabDelMsg[name];
            if (msg && msg.show) {
                const content = msg.msg || '确定关闭页面？';
        ...
        setCloseTab(key, isPush = true) {
            const name = pathName(this.tabList[key]);
