			...
			<ul slot="content">
				<li>
					<dl>
						<dt>产品名称：</dt>
						<dd>
							<h-input ref="keyword" v-model.trim="keyword"  placeholder="产品名称"  icon="android-close" @on-keyup.enter="paramsSub()" @on-click="clear()"></h-input>
						</dd>
					</dl>
				</li>
				<li class="search-wrapper-but">
					<h-button type="primary" @click="paramsSub()">查询</h-button>
					<h-button type="primary" @click="onAdd" v-if="activeRoutersButton.indexOf('ProductAdd') != -1">新增</h-button>
				</li>
			</ul>
		</search-form>
...
<script>
	export default {
				...
				columns:[
					{
						...
						width: 300,
						render: (h,params) => {
	                       const toLink = {path: '/chainspectrum/product/info', query: {id: params.row.id}};
					...
					},
					{
						...
						render: (h, params) => {
							...
							let inChain = null;
							let outChain = null;
							if(this.activeRoutersButton.indexOf('ProductAdd') != -1){
								editBtn = h('i',{
									class: {
										'iconfont': true,
							...
							if(this.activeRoutersButton.indexOf('ProductDel') != -1){
		...
		methods:{
			...
			search() {
				this.loading = true
				let url = '/tm/chainproduct/chainproductlist?pagenum=' + this.pageNum + '&pagesize=' + this.pageSize + '&cpmc=' + encodeURIComponent(this.keyword_copy.trim())
			...
			onAdd(){
				this.$router.push({path: '/chainspectrum/product/edit', query: {id: ''}})
			},
			onEdit(row){
			...
			del(row) {
				let id = row.id
				let url = '/tm/chainproduct/delete?id=' + id
...
.product > .search,.product > .serch .primary-btn {
	line-height: 32px;
	width: 100%;
	display: inline-block;
}
.product > .search .h-input-wrapper{
	width: 200px;
	margin-right: 10px;
	vertical-align: top;
	float: left;
...
.product > .search .title,.product > .search .h-btn{
	float: left;
	margin-bottom: 10px;
}
.product > .search .h-btn{
	margin-right: 10px;
}
</style>