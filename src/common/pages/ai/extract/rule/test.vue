<template>
	<div class="test">
			...
			<search-form :title="name">
				<ul slot="content">
					<li>
						<dl>
							<dt>任务名称：</dt>
							<dd><h-input v-model="req.taskName" placeholder="请输入" icon="android-close" @on-click="shitClearableClick('taskName','req')" ref="taskName"></h-input></dd>
						</dl>
					</li>
					<li>
						<dl>
							<dt>更新人：</dt>
							<dd>
								<Select v-model.trim="req.userId" filterable >
		...
		<h-msgBox title="新建" v-model="isShowMsgBox" @on-close="onCloseMsgBox" class-name="text-file-msg" width="620" height="200">
			<div class="file-form">
				<h-form :model="submit" ref="submitForm" label-position="right" :label-width="100" :rules="modulesRule" @submit.native.prevent>
					...
					<h-form-item label="文件来源：" prop="select">
						<h-select v-model="submit.fileSource" :clearable="false">
							<h-option v-for="(item, k) in fileSource" :key="k" :value="item.dictEntry" >
								{{ item.entryName }}
							</h-option>
						</h-select>
...
<!--									<Upload action="/tm/extractFile/upload" name="files" :multiple="true" :show-upload-list="false" :on-error="uploadError" :uploadAll="true" :on-self-success="uploadSuccess" :before-upload="sequelHandleBeforeUpload" >-->
...
<!--									</Upload>-->
                  <div>
                    <customUpload @change="inputChange" title="上传"></customUpload>
								...
								<template v-else>
									<div><h-button type="primary">文件上传中...</h-button></div>
								</template>
							...
							<template v-else-if="submit.fileSource == 3">
								<h-select v-model="submit.ruleFileId" filterable>
									<h-option v-for="(item, k) in fileList" :key="item.id" :value="item.id" >
										{{ item.fileName }}
									</h-option>
								</h-select>
							</template>
							<template v-else-if="submit.fileSource == 1">
								<h-select v-model="submit.normDataId" filterable>
									<h-option v-for="(item, k) in standard" :key="item.standardId" :value="item.standardId" >
										{{ item.standardName }}
									</h-option>
								</h-select>
							</template>
		...
		<h-msgBox title="设置为标准数据" v-model="isShowStandardBox" @on-close="onCloseStandardMsgBox" class-name="vertical-center-modal text-file-msg" :top="0" width="520" height="100">
			<div class="file-form">
				<h-form :model="submitStandard" ref="submitStandardForm" label-position="right" :label-width="100" :rules="modulesStandardRule" @submit.native.prevent>
  ...
  components: { customUpload },
					...
					render: (h, params) => {
						...
						if(row.taskStatus == 2){
							viewBtn = h('span', {
								...
								on: {
									click: () => {
										this.$router.push('/extract/rule/test/detail?id=' + params.row.id +'&name='+ row.taskName);
									}
								},
							})
							/*standardBtn = h('span', {
								...
								on: {
									click: () => {
										this.activeStandard = params.row;
										this.submitStandard.taskId = params.row.id;
										this.showStandard();
									}
								},
							})*/
						...
						let originalBtn = h('span', {
								...
								on: {
									click: () => {
										this.$router.push('/extract/rule/view?view=' + params.row.id +'');
									}
								},
							})
			...
			this.$refs['submitStandardForm'].validate((valid) => {
				if(valid){
					...
					this.$http.get(url, obj).then((res)=>{
						if(obj.taskId != this.submitStandard.taskId && this.submitStandard.taskId != '' )return;
						let oTmp = res.data;
		...
		onSubmit(){
			this.$refs['submitForm'].validate((valid) => {
				if(valid){
					...
					if(submit.fileSource == 2 && submit.fileIds.length == 0 && submit.id == '' ){
					...
					submit.ruleConfigId =  this.req.ruleConfigId;
					if(this.isPOST)return;
					this.isPOST = true;
					let url = '/tm/testTask/addOrModify';
					this.$http.post(url, submit).then((res)=>{
						let oTmp = res.data;
						if(oTmp.status == this.$api.SUCCESS){
							this.onCloseMsgBox();
							this.getList(true);
    ...
    uploadFile (repeatUploadFileMd5s, fileList) {
      ...
      this.$http.post('/tm/extractFile/upload', formData).then(res => {
		...
		getStandard(){
			let url = '/tm/standardData/getStandard?ruleId=' + this.req.ruleConfigId;
			this.$http.get(url).then((res)=>{
				let oTmp = res.data;
				if(oTmp.status == this.$api.SUCCESS){
	...
	mounted() {
		this.isMounted = true;
		this.$store.commit('SAVE_TAB_NAME',{  path: this.$route.path , name: '配置规则 - 测试'});
		this.init();
		this.getFilelist();
		this.getFileSource();
		this.getStandard();
