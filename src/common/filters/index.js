	...
	function tree(id, array, arr = []) {
		...
		if (ary.length > 0) { //如果有值，采用递归得到他的子元素
			for (let i = 0, len = ary.length; i < len; i++) {
				let children = tree(ary[i].id, array, [...arr]);
				if (children) {
...
export function checkParseFloat(val, integer, decimals) {
	integer--;
	let re = new RegExp("^([1-9]\\d{0," + integer + "}|0)(\\.\\d{1," + decimals + "})?$");
	return re.test(val);
}
...
export function encrypt (str, passKey, type) {
	if (type === 'encryption') {
	  ...
	  let incr = Math.ceil(passKey.length / 2)
	...
	if (type === 'decryption') {
	  ...
	  let sPos = Math.floor(prand.length / 5)
	  let mult = parseInt(prand.charAt(sPos) + prand.charAt(sPos * 2) + prand.charAt(sPos * 3) + prand.charAt(sPos * 4) + prand.charAt(sPos * 5))
	  let incr = Math.round(passKey.length / 2)
	  let modu = Math.pow(2, 31) - 1
