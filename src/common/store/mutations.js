...
const mutations = {
    ...
    'GET_EVENTTYPELIST'(st, arr) {
        st.eventTypeList = arr;
    },
    ...
    'GET_SENTIMENTLIST'(st, arr) {
        st.sentimentList = arr;
    },
    'GET_NEWSIMPORTANCELIST'(st, arr) {
        st.newsImportanceList = arr; 111;
    },
    ...
    'SET_ACTIVE_ROUTERS_BUTTON'(st, path) {
        const button = st.routersButton[pathName(path)];
        st.activeRoutersButton = button || []; // 更新对应页面的按钮权限
    },
    [types.ADD_TAB](st, router) {
        ...
        if (st.routersObj[pathName(path)]) {
          ...
          if (st.tabList.indexOf(path, 0) >= 0) {
            const index = st.tabList.indexOf(path);
    ...
    'DEL_TAB'(st, router) {
        const index = st.tabList.indexOf(router);
        if (index >= 0) {
          const name = pathName(router);
          if (name) {
            st.scrollTop[name] = 0;
          }
    ...
    'SAVE_TAB_NAME'(st, router) {
        ...
        const len = tabList.indexOf(router.path);
        if (len != -1) {
            st.routersObj[pathName(router.path)] = router.name;
            st.tabList = [...tabList];
        }
